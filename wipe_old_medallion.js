const fs = require('fs');

// 1. Wipe old medallion HTML
let html = fs.readFileSync('index.html', 'utf8');
const btnRegex = /<button class="icon-btn shield-btn luxury-medallion-btn"[^>]*>[\s\S]*?<\/button>/;
const newBtnHtml = `<button class="icon-btn shield-btn luxury-medallion-btn" type="button" aria-label="Registry">
          <div class="medallion-rim">
            <div class="medallion-core">
              <svg viewBox="0 0 24 24" class="medallion-symbol">
                <defs>
                  <linearGradient id="medGoldDomed" x1="0%" y1="0%" x2="100%" y2="100%">
                    <stop offset="0%" stop-color="#FDF2D0" />
                    <stop offset="25%" stop-color="#D4AF37" />
                    <stop offset="50%" stop-color="#9E7422" />
                    <stop offset="80%" stop-color="#4A3110" />
                    <stop offset="100%" stop-color="#241604" />
                  </linearGradient>
                  <linearGradient id="medGoldEdge" x1="0%" y1="0%" x2="100%" y2="100%">
                    <stop offset="0%" stop-color="#FFFFFF" />
                    <stop offset="50%" stop-color="#D4AF37" />
                    <stop offset="100%" stop-color="#241604" />
                  </linearGradient>
                </defs>
                <path d="M12 2.5L18.5 5.5V11.5C18.5 15.5 16 19 12 20.5C8 19 5.5 15.5 5.5 11.5V5.5L12 2.5Z" class="medallion-shield-base" />
                <path d="M9.5 11L11.5 13L15 9" class="medallion-shield-check" />
              </svg>
            </div>
          </div>
        </button>`;
html = html.replace(btnRegex, newBtnHtml);
fs.writeFileSync('index.html', html);

// 2. Wipe old medallion CSS
let css = fs.readFileSync('style.css', 'utf8');
css = css.replace(/\/\* ==========================================================================\n   LUXURY VERIFICATION MEDALLION \(Header\)\n   ========================================================================== \*\/[\s\S]*?(?=\/\*|$)/, '');
fs.writeFileSync('style.css', css);

